...
握手后，一条连接只有一个 reader 和一个 writer。`publish`、`subscribe` 以及 reader 对服务端 `PING` 的响应都写入 writer 邮箱；发送调用等待该命令完成一次 socket write，因此同一连接上的 `SUB` 会先于紧随其后的 `PUB`。每个 SID 在进入邮箱前已经保存 subject、queue group 和可选消息上限，writer 从这份状态写出完整安装序列。普通订阅是一条 SUB；request inbox 的 SUB 与 `UNSUB sid 1` 属于同一 writer 命令，中间不会插入另一任务的写入。这个完成信号不表示服务端已经处理命令。
...
request 为每次调用建立一个精确 inbox 订阅。inbox 前缀来自 12 个系统随机字节，连接内序号区分后续请求；writer 连续写出 SUB 与 `UNSUB sid 1`，随后才允许带 reply subject 的 PUB 或 HPUB 进入 socket。同一条响应路径处理普通 MSG、带 headers 的 HMSG 和服务端 503。等待超过 `Options` 中的毫秒数时，请求先取消 inbox 并跨过 PING/PONG 屏障，再把 `Timeout` 交给调用者。
...
服务器地址表分成调用者配置的种子和 INFO 发现的成员。种子始终位于前部且不会被服务器删除；`connect_urls` 每次出现时替换上一份发现列表，同时剔除与种子或自身重复的地址。INFO 不含这个字段时维持原表，显式空数组则清空发现成员。握手期间得到的新地址可以加入同一次交付前的尝试；运行期 INFO 也更新这份表。

每个等待 socket 写入或 PING/PONG 的调用在连接上登记自己的完成邮箱。reader 遇到 EOF 或读错误、writer 遇到写错误时，连接先进入 `Reconnecting`，再以 `Disconnected` 关闭仍未完成的这些邮箱；已经放入完成值的邮箱仍交付成功结果。状态切换之后的新 publish、subscribe、request、flush 和 drain 不会进入旧 writer。普通且仍在接纳消息的订阅留在 SID 表中，它的 subject、queue group 和消息上限可以生成恢复命令；尚未写完 SUB 的 SID、request inbox，以及正在 unsubscribe、drain 或慢消费者退休过程中的 SID 都不会跨连接续接。当前 `with_client` 随后仍结束作用域，尚没有负责退避和重新拨号的监督任务。
